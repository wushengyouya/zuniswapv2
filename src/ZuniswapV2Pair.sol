...
contract ZuniswapV2Pair is ERC20, Math {
    ...
    // 第一个流动性添加用户会少 1000 LP token, 用来 Mint 给 address(0) 地址，减少通货膨胀攻击
    // 攻击者需要多付出 1000 倍金额
    ...
    function mint(address _to) public returns (uint256 liquidity) {
        ...
        if (totalSupply == 0) {
            ...
            _mint(address(0), MINIMUM_LIQUIDITY); // 减少 inflation attack  https://mixbytes.io/blog/overview-of-the-inflation-attack#rec558244009
        } else {
            // 为什么取最小的
            // 用户获得最小的lp token, 多出的无法获取收益
            // 1. 确保按照比例存款 2. 防止价格操纵 3. 维持价格比率
            liquidity = Math.min(
    ...
    function swap(
        ...
        // 交换后取出的金额不能大于pair合约代币余额的总量
        if (amount0Out > reserve0_ || amount1Out > reserve1_) {
            revert InsufficientLiquidity();
        }

        // TODO:为什么没遵守CEI模式，是为了闪电贷功能吗
        // 执行安全转账
        if (amount0Out > 0) {
        ...
        /**
         * 计算用户输入token的金额, 先transfer后再计算的In
         * 交换的当前余额 = 上次存储的余额 + 交换输入的金额 - 输出的金额 (由于不知道用户交换的是哪种token, 计算公式中都减去了输出的金额)
        ...
        // uniswapv2默认收取千分之三的手续费，计算将数据放大1000倍进行运算，以保留后面小数位
        /* 易读的写法, 会丢失小数位
            uint256 fee = amount0In * 3 / 1000;
            uint256 balance0Ajusted = balance0 - fee;
        */
        uint256 balance0Ajusted = (balance0 * 1000) - (amount0In * 3);
        uint256 balance1Ajusted = (balance1 * 1000) - (amount1In * 3);
        console.log(balance0Ajusted, balance1Ajusted, reserve0_, reserve1_);
        console.log(
